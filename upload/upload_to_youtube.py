...
def generate_video_metadata(category: str, num_phrases: int, phrases: list = None):
    ...
    title = f"Russian Learning: {num_phrases} Essential {category} Phrases 🇷🇺"
    ...
    if phrases:
        ...
        for i, phrase in enumerate(phrases[:5], 0):
            ...
            description_lines.append(f"   📍 {phrase['russian']}")
    ...
    description_lines.extend([
        ...
        f"🔔 Subscribe for daily Russian lessons!",
        f"",
        f"📖 Pronunciation Guide:",
        f"   The phonetic spelling in brackets helps you say it correctly!",
        f"",
        f"#LearnRussian #RussianLessons #RussianForBeginners #LanguageLearning",
        f"#Russian #Education #Tutorial #DailyRussian #{category.replace(' ', '')}",
        f"#VelocityRussian #RussianPhrases #SpeakRussian"
    ])
